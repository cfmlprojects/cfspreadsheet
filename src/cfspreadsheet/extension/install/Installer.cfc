<cfcomponent>
	...
	<cffunction name="addFunctionStubs" output="false">
		<cfargument name="functionsDir" />
		<cfset var functions = "" />
		<cfdirectory action="list" directory="#expandPath(functionsDir)#" name="functions">
		<cfloop query="functions">
			<cffile action="write" file="#getLibraryPath()#/function/#name#" output='<cfinclude template="#functionsDir#/#name#" />' />
		</cfloop>
		<cfreturn />
	</cffunction>
	<cffunction name="removeFunctionStubs" output="false">
		<cfargument name="functionsDir" />
		<cfset var functions = "" />
		<cfdirectory action="list" directory="#expandPath(functionsDir)#" name="functions">
		<cfloop query="functions">
	...
	<cffunction name="addMapping" output="false">
		<cfargument name="virtualPath" />
		<cfargument name="physicalPath" />
		<cfadmin action="getMappings" returnVariable="Mappings" type="#request.adminType#" password="#session["password"&request.adminType]#" />
		<cfquery name="isAlreadyMapped" dbtype="query">
			SELECT * FROM Mappings WHERE strphysical =
			<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.physicalPath#" />
		</cfquery>
		<cfif NOT isAlreadyMapped.recordcount>
			<cfadmin action="updatemapping" archive="false" primary="false" trusted="false" virtual="/ext/#arguments.virtualPath#" physical="#arguments.physicalPath#" type="#request.adminType#" password="#session["password"&request.adminType]#" />
		<cfelse>
			<cfthrow type="#variables.extensionTag#.mappingsThereDude" detail="'#arguments.physicalPath#' has already been mapped as a custom tag directory" />
		</cfif>
		<cfadmin action="reload" type="#request.adminType#" password="#session["password"&request.adminType]#" />
	...
	<cffunction name="removeMapping" output="false">
		<cfargument name="physicalPath" />
		<cfadmin action="getMappings" returnVariable="Mappings" type="#request.adminType#" password="#session["password"&request.adminType]#" />
		<cfquery name="isAlreadyMapped" dbtype="query">
			SELECT * FROM Mappings WHERE strphysical =
		...
		<cfif isAlreadyMapped.recordcount>
			<cfadmin action="removemapping" physical="#arguments.physicalPath#" virtual="#isAlreadyMapped.virtual#" type="#request.adminType#" password="#session["password"&request.adminType]#" />
		<cfelse>
			<cfthrow type="#variables.extensionTag#.noMappingToRemove" detail="no mapping for path: #arguments.physicalPath#" />
		</cfif>
		<cfadmin action="reload" type="#request.adminType#" password="#session["password"&request.adminType]#" /> <cfadmin action="getMappings" returnVariable="Mappings" type="#request.adminType#" password="#session["password"&request.adminType]#" />
		<cfquery name="isAlreadyMapped" dbtype="query">
			SELECT * FROM Mappings WHERE strphysical =
			<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.physicalPath#" />
		</cfquery>
